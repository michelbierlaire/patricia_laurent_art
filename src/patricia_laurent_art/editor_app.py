...
from .models import Artwork, LocalizedList, LocalizedText
...
def create_app() -> Flask:
    ...
    app.jinja_loader = ChoiceLoader(
        [app.jinja_loader, FileSystemLoader(str(TEMPLATES_DIR / "editor"))]
    )
    ...
    def site_config() -> str:
        ...
        if request.method == "POST":
            config.site_title = (
                request.form.get("site_title", "").strip() or "Patricia Laurent"
            )
            config.email = request.form.get("email", "").strip()
            config.default_theme = (
                request.form.get("default_theme", "gallery").strip() or "gallery"
            )
            config.default_locale = (
                request.form.get("default_locale", "fr").strip() or "fr"
            )
            config.home.image = (
                request.form.get("home_image", config.home.image).strip()
                or config.home.image
            )
            config.home.title = LocalizedText(
                request.form.get("home_title_fr", "").strip(),
                request.form.get("home_title_en", "").strip(),
            )
            config.home.lead = LocalizedText(
                request.form.get("home_lead_fr", "").strip(),
                request.form.get("home_lead_en", "").strip(),
            )
            config.home.hero_text = LocalizedText(
                request.form.get("hero_text_fr", "").strip(),
                request.form.get("hero_text_en", "").strip(),
            )
            config.home.hero_cta = LocalizedText(
                request.form.get("hero_cta_fr", "").strip(),
                request.form.get("hero_cta_en", "").strip(),
            )
            config.home.intro_title = LocalizedText(
                request.form.get("intro_title_fr", "").strip(),
                request.form.get("intro_title_en", "").strip(),
            )
            config.home.intro_text = LocalizedText(
                request.form.get("intro_text_fr", "").strip(),
                request.form.get("intro_text_en", "").strip(),
            )
        ...
        if request.method == "POST":
            ...
            try:
                ...
                return render_template(
                    ...
                    artwork=artwork,
                    is_new=True,
                    available_images=image_service.list_images(),
                )
    ...
    def edit_artwork(artwork_id: str) -> str:
        ...
        if request.method == "POST":
            updated = _build_artwork_from_form(
                request, allow_id_change=False, artwork_id=artwork_id
            )
            try:
    ...
    def _build_artwork_from_form(
        req, *, allow_id_change: bool, artwork_id: str | None = None
    ) -> Artwork:
        final_id = (
            req.form.get("id", "").strip() if allow_id_change else (artwork_id or "")
        )
        pictures = [
            value.strip() for value in req.form.getlist("pictures") if value.strip()
        ]
        return Artwork(
            id=final_id,
            title=LocalizedText(
                req.form.get("title_fr", "").strip(),
                req.form.get("title_en", "").strip(),
            ),
            description=LocalizedText(
                req.form.get("description_fr", "").strip(),
                req.form.get("description_en", "").strip(),
            ),
            type=LocalizedText(
                req.form.get("type_fr", "").strip(), req.form.get("type_en", "").strip()
            ),
            ...
            keywords=LocalizedList(
                [
                    item.strip()
                    for item in req.form.get("keywords_fr", "").split(",")
                    if item.strip()
                ],
                [
                    item.strip()
                    for item in req.form.get("keywords_en", "").split(",")
                    if item.strip()
                ],
            ),
            notes=LocalizedText(
                req.form.get("notes_fr", "").strip(),
                req.form.get("notes_en", "").strip(),
            ),
            pictures=pictures,
        )
